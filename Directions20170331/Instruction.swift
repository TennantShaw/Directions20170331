...
func move(_ amountToMove: Int, _ fromFacing: CardinalDirection, _ from: Location) -> Location {
    switch fromFacing {
    case .north:
...
func calculateBlocksMoved(startLocation: Location, finalLocation: Location) -> Int {
    let pointX = abs((finalLocation.x - startLocation.x))
    let pointY = abs((finalLocation.y - startLocation.y))
    let numberOfBlocks = pointX + pointY
    
    return numberOfBlocks
}

func movement(startLocation: Location, fromFacing: CardinalDirection, instruction:
    (Instruction, amountToMove: Int) )-> (Location, CardinalDirection) {
    let newInstruction = turn(instruction.0, fromFacing)
    let newLocation = move(instruction.amountToMove, newInstruction, startLocation)
    
    return (newLocation, newInstruction)
}

func travel(startLocation: Location, fromFacing: CardinalDirection, instruction:
    [(Instruction, Int)]) -> (Location, CardinalDirection) {
    var facingDirection = fromFacing
    var location = startLocation
    
    for (instructions, amountToMove) in instruction {
        facingDirection = turn(instructions, facingDirection)
        location = move(amountToMove, facingDirection, location)
    }
...
func travel2(startLocation: Location, instruction: [(CardinalDirection, Int)]) -> Location {
    var distance = startLocation
    
    for (cardinalDirection, amountToMove) in instruction {
        switch cardinalDirection {
        case .north:
            distance.x = distance.x + amountToMove
        case .east:
            distance.y = distance.y + amountToMove
        case .south:
            distance.x = distance.x - amountToMove
        case .west:
            distance.y = distance.y - amountToMove
...
func travel3(_ startingPoint: Location, _ instructions: [(BadDirections, Int)]) -> (Location, BadDirections) {
    var facingDirection: BadDirections = .north
    var startLocation = startingPoint
    
    for (instruction, amountToMove) in instructions {
        switch instruction {
        case .north:
            startLocation.y = startLocation.y + amountToMove
            facingDirection = .north
        case .east:
            startLocation.x = startLocation.x + amountToMove
    ...
    return (startLocation, facingDirection)
}
